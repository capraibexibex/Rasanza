...
const float VOLTAGE_UPTHRESHOLD = 0.54;
...
void displayWelcomeMessageWithGPS(float roundedTemp, float roundedHumidity) {
  ...
  // Attempt to read and parse the GPS data
  char c = GPS.read(); // Read GPS data
  if (GPS.newNMEAreceived()) {
    if (GPS.parse(GPS.lastNMEA())) { // When new data is parsed successfully
      if (GPS.fix) {
        // Print the GPS location on the last line
        ...
        // Display a waiting message or error if no fix is found
...
int timoutRestriction = 60000; // ms until GATE1 goes to reset
const long countdownDuration = 60000; // 10 seconds countdown
